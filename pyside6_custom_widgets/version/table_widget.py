...
class CustomTableWidget(QWidget):
    ...
    #         else:
    #             for col_idx, instance in enumerate(data):
    #                 self.table.setItem(row_position, col_idx, QTableWidgetItem(str(instance.id)))
    #                 self.table.setColumnHidden(col_idx, True)  
    #                 self.table.setItem(row_position,col_idx, QTableWidgetItem(instance.title))
    ...
    def populate_table(self, data):
        ...
        Args:
            data (list): A list of instances from a SQLAlchemy model.
        """
        self.table.setRowCount(0)  # Clear existing rows

        for row_idx, model_instance in enumerate(data):
            row_position = self.table.rowCount()
            self.table.insertRow(row_position)

            # Loop through headers to populate table with model attributes
            for col_idx, header in enumerate(self.headers):
                # Skip the "Actions" column
                if header == "Actions":
                    continue

                # Dynamically fetch the value from the model instance using getattr
                value = getattr(model_instance, header, "")
                formatted_value = self.formatter[col_idx](value) if self.formatter and col_idx in self.formatter else str(value)
                self.table.setItem(row_position, col_idx, QTableWidgetItem(formatted_value))
